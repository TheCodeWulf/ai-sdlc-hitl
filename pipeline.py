...
def after_code(artifacts, state):
    ...
    # extract the runnable .py from the Code agent's markdown -> src/<feature>.py
    code_path, code_text = extract_code_file(artifacts["code"], fallback_name=epic)
    files = {
        code_path: code_text,                                   # real source file
        f"docs/{code_path.split('/')[-1].replace('.py','')}_change.md": artifacts["code"],  # design + self-review
    }
    try:
        gh = GitHubClient()
