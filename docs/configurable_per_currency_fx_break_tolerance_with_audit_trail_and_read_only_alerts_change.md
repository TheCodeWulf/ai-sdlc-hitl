**## Design note**  
Implement a tiny Flask service with SQLAlchemy models for `fx_tolerance` and `fx_tolerance_audit`.  Expose CRUD endpoints guarded by a simple role‑header (`X-User-Role`).  Provide a reconciliation helper that filters a supplied list of FX breaks using the per‑currency tolerance (or a global default) and a read‑only alerts endpoint that returns breaks exceeding their tolerance.  Every insert or update of a tolerance writes an immutable audit record.
...
**## Self-review**  
- **Scope adherence:** Implements tolerance storage, CRUD with role checks, fallback default, audit‑log on every change, reconciliation filter, and read‑only alerts – exactly the four stories.  
- **Immutability:** `FxToleranceAudit` records are only inserted; no update/delete routes are exposed, satisfying the immutable audit requirement.  
- **Simplicity:** Uses an in‑memory SQLite DB and a minimal Flask app, keeping the implementation lightweight while covering all acceptance criteria.