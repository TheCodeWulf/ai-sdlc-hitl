## Design note  
A lightweight Flask service stores per‑currency tolerances in an SQLite table (`tolerances`).  
* The UI is a single page (`/config`) that lists the three supported currencies (USD, EUR, JPY) and allows the manager to edit each value.  
* A hard‑coded `DEFAULT_TOLERANCE` is applied automatically for any currency not explicitly configured.  
* All changes are persisted to the database so they survive restarts.  
...
* **Persistence** – Uses SQLite; tolerances survive restarts and are stored in a single table.  
* **Default handling** – `get_tolerance` returns `DEFAULT_TOLERANCE` when a currency has no record.  
* **UI** – A single page shows current values and allows editing; minimal but functional.  
* **Scope** – No audit logging or alerting logic added, staying within Story 1.