## Design note  
A lightweight in‑memory module that keeps a per‑currency tolerance map, a default tolerance, and an append‑only audit log.  
Breaks are evaluated against the tolerance; those below threshold are hidden, others trigger a simple alert.  
Missing‑position breaks bypass tolerance checks.
...
## Self‑review  
- **Scope‑constrained**: Only the core tolerance, alerting, and audit logic is implemented; no external DB or UI.  
- **Immutability**: Audit entries are stored as dataclasses and never modified after creation.  
- **Extensibility**: Adding new currencies or changing default tolerance is straightforward via `set_tolerance`.